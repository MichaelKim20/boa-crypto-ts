...
export class Scalar
{
    ...
    constructor (data: Buffer | string, endian: Endian = Endian.Big)
    {
        ...
        {
            this.data = Buffer.alloc(sodium.crypto_core_ed25519_SCALARBYTES);
            this.fromBinary(data, endian);
        }
        if (this.data.length !== sodium.crypto_core_ed25519_SCALARBYTES)
    ...
    {
        if (bin.length !== sodium.crypto_core_ed25519_SCALARBYTES)
    ...
    {
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_reduce(hash.data)));
    }
    ...
    {
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_invert(this.data)));
    }
    ...
    {
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_random()));
    }

    private static ED25519_L = Buffer.from("edd3f55c1a631258d69cf7a2def9de1400000000000000000000000000000010", "hex");
    ...
    {
        let ret = new Point(Buffer.from(sodium.crypto_scalarmult_ed25519_base_noclamp(this.data)));
        if (!ret.isValid)
    ...
    public static add (x: Scalar, y: Scalar): Scalar
    {
        ...
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_add(x.data, y.data)));
    }
    ...
    public static sub (x: Scalar, y: Scalar): Scalar
    {
        ...
        if (y.isNull())
            return new Scalar(x.data);
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_sub(x.data, y.data)));
    }
    ...
    public static mul (x: Scalar, y: Scalar): Scalar
    {
        ...
        if (y.isNull())
            return new Scalar(x.data);
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_mul(x.data, y.data)));
    }
    ...
    {
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_negate(this.data)));
    }
    ...
    {
        return new Scalar(Buffer.from(sodium.crypto_core_ed25519_scalar_complement(this.data)));
    }
...
export class Point
{
    ...
    constructor (data: Buffer | string, endian: Endian = Endian.Big)
    {
        ...
        {
            this.data = Buffer.alloc(sodium.crypto_core_ed25519_BYTES);
            this.fromBinary(data, endian);
        }
        if (this.data.length !== sodium.crypto_core_ed25519_BYTES)
            throw new Error("The size of the data is abnormal.");
    }
    ...
    {
        if (bin.length !== sodium.crypto_core_ed25519_BYTES)
            throw new Error("The size of the input data is abnormal.");
    ...
    public isValid (): boolean
    {
        return sodium.crypto_core_ed25519_is_valid_point(this.data);
    }
    ...
    {
        return new Point(Buffer.from(sodium.crypto_core_ed25519_random()));
    }
    ...
    public static add (p: Point, q: Point): Point
    {
        ...
        return new Point(Buffer.from(sodium.crypto_core_ed25519_add(p.data, q.data)));
    }
    ...
    public static sub (p: Point, q: Point): Point
    {
        ...
        if (q.isNull())
            return new Point(p.data);
        return new Point(Buffer.from(sodium.crypto_core_ed25519_sub(p.data, q.data)));
    }
    ...
    {
        return new Point(Buffer.from(sodium.crypto_scalarmult_ed25519_noclamp(s.data, n.data)));
    }
    ...
    public isNull (): boolean
    {
        return (Buffer.compare(this.data, Buffer.alloc(sodium.crypto_core_ed25519_BYTES)) == 0);
    }
    ...
    {
        return new Point(Buffer.alloc(sodium.crypto_core_ed25519_BYTES));
    }
}
